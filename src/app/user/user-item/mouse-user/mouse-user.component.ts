...
import { ItemService } from './usermoniter-service';
...
@Component({
  ...
  templateUrl: './mouse-user.component.html',
  styleUrls: ['./mouse-user.component.css']
})
export class MouseUserComponent {
  ...
  private getItemList(){
    this.itemService .getItemList().subscribe(data =>{
      this.item =data.filter(item => item.categoryID === 900);
    })
  }
