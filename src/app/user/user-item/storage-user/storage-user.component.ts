...
import { ItemService } from './userstorage-service';
import { cartCount } from '../../cart.count';
import  AOS from 'aos';
import { Item } from './userstorage-model';

@Component({
  selector: 'app-storage-user',
  templateUrl: './storage-user.component.html',
  styleUrls: ['./storage-user.component.css']
})
export class StorageUserComponent {
  ...
  private getItemList(){
    this.itemService .getItemList().subscribe(data =>{
      this.item =data.filter(item => item.categoryID === 600);
    })
  }
