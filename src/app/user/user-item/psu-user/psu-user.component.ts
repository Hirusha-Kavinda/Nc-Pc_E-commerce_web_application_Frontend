...
import { ItemService } from './userpsu-service';
import { cartCount } from '../../cart.count';
import  AOS from 'aos';
import { Item } from './userpsu-model';

@Component({
  selector: 'app-psu-user',
  templateUrl: './psu-user.component.html',
  styleUrls: ['./psu-user.component.css']
})
export class PsuUserComponent {
  ...
  private getItemList(){
    this.itemService .getItemList().subscribe(data =>{
      this.item =data.filter(item => item.categoryID === 800);
    })
  }
