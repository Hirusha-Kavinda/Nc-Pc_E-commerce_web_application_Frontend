...
import  AOS from 'aos';
import { Item } from './usermemory-model';
import { ItemService } from './usermemory-service';

@Component({
  selector: 'app-memory-user',
  templateUrl: './memory-user.component.html',
  styleUrls: ['./memory-user.component.css']
})
export class MemoryUserComponent {
  ...
  private getItemList(){
    this.itemService .getItemList().subscribe(data =>{
      this.item =data.filter(item => item.categoryID === 500);
    })
  }
