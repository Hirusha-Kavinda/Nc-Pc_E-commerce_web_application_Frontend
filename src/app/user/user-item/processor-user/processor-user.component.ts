...
import { ItemService } from './userprocessor-service';
import { Item } from './userprocessor-model';
import { cartCount } from '../../cart.count';
import  AOS from 'aos';

@Component({
  selector: 'app-processor-user',
  templateUrl: './processor-user.component.html',
  styleUrls: ['./processor-user.component.css']
})
export class ProcessorUserComponent {
  ...
  private getItemList(){
    this.itemService .getItemList().subscribe(data =>{
      this.item =data.filter(item => item.categoryID === 300);
    })
  }
